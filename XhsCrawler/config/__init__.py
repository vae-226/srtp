...
_TOML_KEY_MAP = {
    ...
    "note_urls":         "XHS_SPECIFIED_NOTE_URL_LIST",
    "creator_urls":      "XHS_CREATOR_ID_LIST",
    "login":             "LOGIN_TYPE",
    "cookie":            "COOKIES",
...
def _set(name: str, value: object) -> None:
    """设置当前模块级变量"""
    globals()[name] = value
    # 同步到 base_config / xhs_config，使 from config.xxx import YYY 也能拿到新值
    from . import base_config, xhs_config
    for mod in (base_config, xhs_config):
        if hasattr(mod, name):
...
def apply_cli_args(args) -> None:
    ...
    if command == "search":
        ...
        if sort_type is not None:
            _set("SORT_TYPE", sort_type)

    # detail 专属
    if command == "detail":
        urls = getattr(args, "urls", None)
        if urls:
            _set("XHS_SPECIFIED_NOTE_URL_LIST", list(urls))

    # creator 专属
    if command == "creator":
        urls = getattr(args, "urls", None)
        if urls:
            _set("XHS_CREATOR_ID_LIST", list(urls))
