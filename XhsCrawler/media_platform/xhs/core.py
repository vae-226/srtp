...
from tools.clean_writer import write_search_result, write_detail_result, write_creator_result
...
class XiaoHongShuCrawler(AbstractCrawler):
    ...
    def __init__(self) -> None:
        ...
        # Accumulate raw data for clean_writer output
        self._raw_notes: List[Dict] = []
        self._raw_comments: Dict[str, List[Dict]] = {}
    ...
    async def search(self) -> None:
        ...
        for keyword in config.KEYWORDS.split(","):
                ...
                try:
                    ...
                    for note_detail in note_details:
                        if note_detail:
                            ...
                            self._raw_notes.append(note_detail)
                    page += 1
            ...
            # Write clean JSON for this keyword
            if self._raw_notes:
                try:
                    write_search_result(keyword, self._raw_notes, self._raw_comments)
                except Exception as e:
                    utils.logger.error(f"[XiaoHongShuCrawler.search] clean_writer error: {e}")
                self._raw_notes = []
                self._raw_comments = {}
    ...
    async def get_creators_and_notes(self) -> None:
        ...
        for creator_url in config.XHS_CREATOR_ID_LIST:
            ...
            # Write clean JSON for this creator
            if self._raw_notes and creator_raw:
                try:
                    write_creator_result(creator_raw, self._raw_notes, self._raw_comments)
                except Exception as e:
                    utils.logger.error(f"[XiaoHongShuCrawler.get_creators_and_notes] clean_writer error: {e}")
    ...
    async def get_specified_notes(self):
        ...
        # Write clean JSON for detail mode
        if self._raw_notes:
            try:
                write_detail_result(self._raw_notes, self._raw_comments)
            except Exception as e:
                utils.logger.error(f"[XiaoHongShuCrawler.get_specified_notes] clean_writer error: {e}")
    ...
    async def get_comments(self, note_id: str, xsec_token: str, semaphore: asyncio.Semaphore):
        ...
        async with semaphore:
            ...
            async def _comment_callback(nid: str, comments: List[Dict]):
                """Wrap store callback to also collect raw comments."""
                await xhs_store.batch_update_xhs_note_comments(nid, comments)
                self._raw_comments.setdefault(nid, []).extend(comments)
