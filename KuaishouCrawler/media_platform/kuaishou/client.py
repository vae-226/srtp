...
class KuaiShouClient(AbstractApiClient, ProxyRefreshMixin):
    ...
    async def post(self, uri: str, data: dict) -> Dict:
        """Send GraphQL POST via browser fetch for reliable authentication & connectivity"""
    ...
    async def get_creator_info(self, user_id: str) -> Dict:
        ...
        return visionProfile.get("visionProfile", {}).get("userProfile")
    ...
    async def get_all_videos_by_creator(
        ...
        while pcursor != "no_more":
            if max_count > 0 and len(result) >= max_count:
                utils.logger.info(
                    f"[KuaiShouClient.get_all_videos_by_creator] Reached max_count limit ({max_count}), stopping."
                )
                break
            ...
            # Trim to max_count if needed
            if max_count > 0 and len(result) + len(videos) > max_count:
                videos = videos[: max_count - len(result)]

            utils.logger.info(
                f"[KuaiShouClient.get_all_videos_by_creator] got user_id:{user_id} videos len : {len(videos)}, total: {len(result) + len(videos)}/{max_count if max_count > 0 else '∞'}"
            )
