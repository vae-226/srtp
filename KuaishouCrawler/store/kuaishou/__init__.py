...
class KuaishouStoreFactory:
    ...
    def create_store() -> "AbstractStore":
        ...
        if not store_class:
            raise ValueError(
                f"[KuaishouStoreFactory.create_store] Invalid save option: {config.SAVE_DATA_OPTION}. "
                f"Only supported: {', '.join(KuaishouStoreFactory.STORES.keys())}"
            )
        return store_class()
