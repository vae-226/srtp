...
class DouYinCrawler(AbstractCrawler):
    ...
    async def start(self) -> None:
        ...
        utils.logger.info("[DouYinCrawler] 开始启动 Playwright...")
        async with async_playwright() as playwright:
            utils.logger.info("[DouYinCrawler] Playwright 启动成功")
            # 根据配置选择启动模式
            if config.ENABLE_CDP_MODE:
                ...
                utils.logger.info("[DouYinCrawler] 开始启动浏览器实例...")
                ...
                utils.logger.info("[DouYinCrawler] 浏览器启动成功，添加反检测脚本...")
                # stealth.min.js is a js script to prevent the website from detecting the crawler.
                await self.browser_context.add_init_script(path="libs/stealth.min.js")

            utils.logger.info("[DouYinCrawler] 创建新页面...")
            self.context_page = await self.browser_context.new_page()
            utils.logger.info("[DouYinCrawler] 开始导航到抖音首页...")
            try:
                await self.context_page.goto(self.index_url, timeout=60000, wait_until="domcontentloaded")
                utils.logger.info("[DouYinCrawler] 抖音首页加载完成")
            except Exception as e:
                utils.logger.error(f"[DouYinCrawler] 导航到抖音首页失败: {e}")
                # 即使导航失败，也继续尝试，因为页面可能部分加载
                utils.logger.info("[DouYinCrawler] 继续执行...")
    ...
    async def create_douyin_client(self, httpx_proxy: Optional[str]) -> DouYinClient:
        ...
        if not user_agent and last_error:
            raise last_error
