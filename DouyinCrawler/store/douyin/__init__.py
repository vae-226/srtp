...
class DouyinStoreFactory:
    ...
    def create_store() -> "AbstractStore":
        ...
        if not store_class:
            raise ValueError(
                f"[DouyinStoreFactory.create_store] Invalid save option: {config.SAVE_DATA_OPTION}. "
                f"Only supported: {', '.join(DouyinStoreFactory.STORES.keys())}"
            )
        return store_class()
